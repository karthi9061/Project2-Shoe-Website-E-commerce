...
const MenShoesPage = ({ shoes, loadMoreShoes }) => {
  ...
  const handleAddToFavorites = (shoe) => {
    ...
    if (favoriteItems.some((item) => item.id === shoe.id)) {
      ...
      setFeedback({
        type: "favorites",
        message: `${shoe.name} removed from favorites!`,
      });
    } else {
      addToFavorites(shoe);
      setFeedback({
        type: "favorites",
        message: `${shoe.name} added to favorites!`,
      });
    }
    setTimeout(() => setFeedback(null), 3000);
  };
    ...
    <div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {shoes.map((shoe, index) => (
          ...
          >
            <img
              src={`http://localhost:8080${shoe.imagePath}`}
              alt={shoe.name}
            ...
            <div className="mt-4">
              ...
              <p className="font-medium text-gray-600 mt-2">
                Colors: {shoe.color?.join(", ") || "No colors available"}
              </p>
              <p className="text-gray-600 mt-2">
                Sizes: {shoe.size?.join(", ") || "No sizes available"}
              </p>
              <div className="mt-4 flex justify-between">
                ...
                <Link
                  to={`/shoe/${shoe.id}`}
                  className="bg-green-500 text-white px-4 py-2 rounded-md transition duration-300 hover:bg-green-600 flex items-center space-x-2"
                >
